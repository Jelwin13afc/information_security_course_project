<?php
$connection = mysqli_connect("localhost", "root", "", "online_bank");
$con = mysqli_connect("localhost","root","","db_transactions");
session_start();
if(isset($_POST['submit'])){
  ...
  $mobileregex = "/^[6-9][0-9]{9}$/";
	...
	$phone = $_POST['phone'];
    $s = mysqli_multi_query($connection, "insert into login(id, username, pwd, account_no, role) values ('$email', '$email',  '$password', '$account_no', '$role'); insert into register(account_no,card_no, firstname, lastname, email, phone, acc_type, address1, address2, city, state, country, father, nationality,balance) values ('$account_no','$card_no', '$fname',  '$lname',  '$email',  '$phone',  '$acc_type',  '$address1',  '$address2',  '$city',  '$state',  '$country',  '$father',   '$nationality','$balance');");
